...
const decimalToHex = decimal => decimal.toString(16).padStart(2, "0").toUpperCase();
...
const getFieldValue = (fields, data, field) => {
	...
	// Parse date fields
	if (value && value.length !== field.length && ["date", "dateWithYear"].indexOf(field.type) !== -1) {
		let date = moment.utc(value, moment.ISO_8601, true);
		if (date.isValid()) {
			value = field.type === "dateWithYear" ? date.format("YYDDDD").substr(1) : date.format("DDDD");
		}
		else {
...
module.exports = data => {
	...
	for (let leg of data.legs) {
		for (let field of fields.filter(f => !f.isSecurityField && (legIndex === 0 || !f.unique))) {
